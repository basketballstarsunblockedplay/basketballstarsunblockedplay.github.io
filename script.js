document.addEventListener("DOMContentLoaded", () => {
  ...
  if (playNowBtn) {
    playNowBtn.addEventListener("click", () => {
      ...
      const mainGameUrl = "https://basketballstarsonline.github.io/file/"
  ...
  function createHotGameCard(gameName) {
    ...
    // Redirect Basketball Stars Unblocked to its specific game page
    if (gameName === "Basketball Stars Unblocked") {
      gameUrl = `games/${generateSlug(gameName)}.html`
    }
...
`
  ...
  gameCards.forEach((card) => {
    card.addEventListener("click", function (event) {
      ...
      const gameUrl = `games/${generateSlug(gameName)}.html`
...
`
  ...
  const popularGames = [
    "Basketball Stars Unblocked", // Changed to Basketball Stars Unblocked
    "Traffic Rider Unblocked",
    "Flip Bros Unblocked",
    "Super Mario Bros Unblocked",
    "Temple Run 2 Unblocked",
  ...
  function loadGamesForCategory(category) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (gameCard) {
        ...
        console.error(`Failed to create game card for: ${games[i]}`)
      }
    }
